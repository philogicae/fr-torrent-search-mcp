...
class YggTorrentApi(BaseTorrentApi):
    ...
    def download_torrent_file_bytes(self, torrent_id: str) -> bytes | None:
        """
        Not supported for YggTorrent - only magnet links are available.
        ...
        Returns:
            None (not supported).
        """
        logger.warning("YggTorrent only supports magnet links, not torrent file bytes.")
        return None
    ...
    def get_magnet_link(self, torrent_id: str) -> str | None:
        ...
        YggTorrent API returns magnet links directly.
        ...
        if not self.enabled:
            return None
        magnet = self._request("GET", f"torrent/{torrent_id[len(self.id_prefix) :]}")
        if magnet:
            if isinstance(magnet, bytes):
